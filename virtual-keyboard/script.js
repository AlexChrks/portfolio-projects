const Keyboard = {
    ...
    _createKeys() {
        ...
        if (this.properties.language === 'en' && this.properties.shift === false && this.properties.capsLock === false) {
            keyLayout = [
                ...
                "done", "z", "x", "c", "v", "b", "n", "m", ",", ".", "/",
                `${this.properties.language}`, "shift", "space", "sound", "speech", "arrow-left", "arrow-right"
            ];
        ...
        keyLayout.forEach(key => {
            ...
            switch (key) {
                case "backspace":
                    ...
                    keyElement.addEventListener('click', () => {
                        if (this.properties.sound === true) {
                            let audio = document.querySelector('.backspace');
                            audio.currentTime = 0;
                            audio.play();
                        }
                ...
                case "caps":
                    keyElement.classList.add("keyboard-key-wide", "caps-nonactive");
                    keyElement.innerHTML = createIconHTML("keyboard_capslock");
                    keyElement.classList.toggle("caps-active", this.properties.capsLock);
                    keyElement.setAttribute('data', 'capslock')

                    const caps = () => {
                        if (this.properties.sound === true) {
                            let audio = document.querySelector('.caps');
                            audio.currentTime = 0;
                            audio.play();
                        }
                        document.querySelector(".use-keyboard-input").focus();
                ...
                case "shift":
                    keyElement.classList.add("keyboard-key-wide", "caps-nonactive");
                    keyElement.classList.toggle("caps-active", this.properties.shift);
                    keyElement.innerHTML = createIconHTML("arrow_upward");
                    keyElement.setAttribute('data', 'shift');

                    const shift = () => {
                        if (this.properties.sound === true) {
                            let audio = document.querySelector('.shift');
                            audio.currentTime = 0;
                            audio.play();
                        }
                        document.querySelector(".use-keyboard-input").focus();
    ...
    soundMainKeys() {
        if (this.properties.language === 'ru' && this.properties.sound === true) {
            const audio = document.querySelector(`.mainKeysRu`);
            if (!audio) return;
            audio.currentTime = 0;
            audio.play();
        } else if (this.properties.language === 'en' && this.properties.sound === true) {
            const audio = document.querySelector(`.mainKeysEn`);
            if (!audio) return;
            audio.currentTime = 0;
            audio.play();
        }
